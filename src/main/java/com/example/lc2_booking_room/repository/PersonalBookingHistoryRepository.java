...
public interface PersonalBookingHistoryRepository extends JpaRepository<com.example.lc2_booking_room.model.ReservationEvent, Long> {
    ...
    @Query(
      value = """
        SELECT
          ...
          STRING_AGG(
            FORMAT(t.start_time, 'HH:mm') + '-' + FORMAT(t.end_time, 'HH:mm'),
            ', '
          ) WITHIN GROUP (ORDER BY t.start_time) AS timeRanges,
        ...
        JOIN dbo.reservation_slots rs
             ON rs.reservation_id = r.reservation_id AND rs.is_active = 1
        JOIN dbo.time_slots t
