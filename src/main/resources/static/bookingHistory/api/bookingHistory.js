...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // helper: ISO -> "dd/mm/BBBB HH:mm" (พ.ศ.) สำหรับไทม์ไลน์
  ...
  // กัน HTML injection เวลาแสดงเหตุผล
  function escHtml(s) {
    return String(s ?? "")
      .replace(/&/g, "&amp;").replace(/</g, "&lt;")
  ...
  // (สำรอง) บีบรายการเวลาที่เป็นข้อความให้เหลือช่วงเดียว
  ...
  function buildTimeline(step, finalStatus, ts = {}) {
    const hasReturn =
      Boolean(ts.returnedAt) || step === "RETURNED_FOR_FIX" || step === "RESUBMITTED";
    ...
    const LABELS = {
      ...
      DECIDE:
        finalStatus === "APPROVED"  ? "อนุมัติแล้ว" :
        finalStatus === "REJECTED"  ? "ถูกปฏิเสธ"   :
        finalStatus === "CANCELLED" ? "ยกเลิก"      : "ผลการยื่นคำร้อง"
    };

    // เวลาที่จะโชว์ใต้แต่ละจุด
    const TIME = {
    ...
    const classes = ORDER.map((k, i) => {
      ...
      if (finalStatus !== "PENDING") return "completed";
      if (i < idxNow)  return "completed";
      if (i === idxNow) return "active";
      return "pending";
    });

    // เรนเดอร์โหนด + meta ใต้หัวข้อ
    return ORDER.map((k, i) => {
      const label = LABELS[k];
      const at = TIME[k];
      const timeDiv = at ? `<div class="status-time">${formatThaiDateTimeShort(at)}</div>` : "";

      // บันทึก “ถูกส่งกลับแก้ไข” ใต้ SUBMITTED เฉพาะมีจริง
      const subNote =
        (k === "SUBMITTED" && hasReturn && ts.returnedAt)
          ? `<div class="status-subnote">ถูกส่งกลับแก้ไข • ${formatThaiDateTimeShort(ts.returnedAt)}</div>`
          : "";

      // เหตุผลปฏิเสธ ใต้จุดสุดท้าย เมื่อ REJECTED
      const reasonDiv =
        (k === "DECIDE" && finalStatus === "REJECTED" && ts.rejectReason)
          ? `<div class="status-reason">เหตุผล: ${escHtml(ts.rejectReason)}</div>`
          : "";
  ...
  function renderDetail(item, full) {
    ...
    // ช่วงเวลาสรุปเดียว + tooltip รายการเต็ม
    const slotsSpan =
    ...
    // สร้างไทม์ไลน์ (ใส่ timestamp/เหตุผลให้ครบ)
    const timeline = buildTimeline(item.step, item.finalStatus, {
      createdAt:       full?.createdAt,
      staffReviewedAt: full?.staffReviewedAt,
      returnedAt:      full?.returnedAt,       // << กรณีตีกลับ
      resubmittedAt:   full?.resubmittedAt,    // << กรณีส่งคำร้องใหม่
      headReviewedAt:  full?.headReviewedAt,
      headDecidedAt:   full?.headDecidedAt,
      approvedAt:      full?.approvedAt,
      rejectedAt:      full?.rejectedAt || full?.headDecidedAt,
      cancelledAt:     full?.cancelledAt,
      rejectReason:    full?.rejectReason      // << เหตุผลการปฏิเสธ
    });
      ...
      try {
        ...
        if (btn) {
          btn.addEventListener("click", () =>
            window.bookingHistoryAPI.openCancelModal(
              item.reservationId,
              div,
              { apiBase: API_BASE, authHeaders: getAuthHeaders() }
            )
          );
        }
      }
  ...
  function renderPagination(page, totalPages, onGo) {
    ...
    if (!pager.isConnected) wrapper.insertAdjacentElement("afterend", pager);
  }
